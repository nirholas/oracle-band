# Integrating OracleBand

## From Solidity

```solidity
import {OracleBandHook} from "oracle-band/src/hooks/OracleBandHook.sol";
...
hook.configure(key, OracleBandHook.Config({ /* see the README */ }));
...
const address = hookAddress("oracle-band", 8453);              // Base
...
```solidity
IHookMetadata(hookAddress).hookName();   // "OracleBand"
...
More at https://oracle-band.pages.dev.
